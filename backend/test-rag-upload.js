...
This document contains important legal information for testing purposes.
    ...
    const document = uploadData.document || uploadData; // Handle both formats
    if (document.id) {
    ...
    if (listResponse.ok) {
      ...
      const documents = listData.documents || listData;
      const uploadedDoc = documents.find(doc => doc.id === document.id);
      if (uploadedDoc) {
